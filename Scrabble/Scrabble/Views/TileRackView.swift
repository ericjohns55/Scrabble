...
struct TileRackView: View {
    ...
    @Binding var tileRackUuids: [UUID]
    ...
    var body: some View {
        VStack {
            GeometryReader { geo in
                ...
                let tileCount = tiles.filter { $0.tileState == .inPlayerHand || $0.tileState == .placedByPlayer }.count
                ...
                ZStack {
                    ...
                    ForEach($tiles) { $tile in
                        if (tileRackUuids.contains($tile.id) && tile.tileState == .inPlayerHand) {
    ...
    private func rackPosition(for tile: Tile, tileSize: CGFloat, spacing: CGFloat) -> CGPoint {
        let index = tileRackUuids.firstIndex(of: tile.id)!
